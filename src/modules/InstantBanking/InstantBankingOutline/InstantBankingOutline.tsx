...
import { InstantBankingServiceBeneFits, InstantBankingServiceConstants } from '@/constants/instant-banking.constant';
...
const InstantBankingOutline: React.FC = () => {
  const outlineItems = [
    {
      title: 'Penny Testing',
      description: "Drop a penny to verify if the bank account exists & match the beneficiary's name to that at the bank."
    },
    {
      title: 'Penniless Testing',
      description: 'Utilize the latest industry innovation to do an instant penniless bank account verification. *Limited coverage'
    },
    {
      title: 'UPI ID',
      description: "Check if a UPI ID exists and get the beneficiary's name from the bank directly."
    },
    {
      title: 'IFSC Code',
      description: "The most up-to-date list of IFSC codes for verification. Now effortlessly match the beneficiary's name to the bank account."
    }
  ];
  ...
  return (
    <div className="flex flex-col items-center text-center px-4 py-16">
      ...
      <p className="max-w-2xl mb-12">
        Paybolt payment link solution helps businesses create and share payment links on-the-go via Email, SMS or WhatsApp to receive payments in no-time. Not just businesses, a homeprenuer or freelancer or any online seller can collect payments online through a payment link solution.
      </p>
          ...
          <React.Fragment key={index}>
            <div className="flex items-start gap-6  mb-4">
              <div className="flex-shrink-0 mr-4 flex items-center w-36">
                <span className="font-semibold mr-2 text-left w-full">{item.title}</span>
              </div>
